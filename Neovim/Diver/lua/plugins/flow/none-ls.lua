return {
  ...
  -- Create MasonUpdate command
    ...
    mason_lspconfig.setup({
       ensure_installed = {
    "pyright", "solargraph", "ts_ls", "gopls", "jdtls", "clangd",
    "omnisharp", "dockerls", "docker_compose_language_service",
    "jsonls", "yamlls", "matlab_ls", "r_language_server", "efm"},
  ...
  require("none-ls-luacheck.diagnostics.luacheck"),
    ...
    extra_args = {"--format='{{.LineNumber}}:{{.Rule}}:{{.Violation}}\n'", "$FILENAME"},
        ...
        require('null_ls').builtins.diagnostics.phpcs.with({
            ft = {"php"},
            cmd = {"phpcs"},
            extra_args = {"--report=json", "-q", "-s", "--runtime-set", "ignore_warnings_on_exit", "1", "--runtime-set",
                        "ignore_errors_on_exit", "1", "--stdin-path=$FILENAME", "--basepath="},
                }),
    ...
    require('null_ls').builtins.formatting.isort.with({
      ...
      extra_args = { "-i", "2", "-ci", "-sr", "-bn", "-fn", "-kp", "--shellopts", "braces" },
        ...
        ft = {"sql"},
        cmd = {"sqlfluff"},
        extra_args = { "--dialect", "postgres", "lint", "--disable-progress-bar", "-f",
                        "github-annotation", "-n", "$FILENAME" },
    }),
...
mason_lspconfig.setup_handlers({
  function(server_name)
    ...
    -- Add more server-specific configurations here if needed
