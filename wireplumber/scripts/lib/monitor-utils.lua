...
function create_cam_node(cam_data) ---@param cam_data WPCameraData
    local api = cam_data.properties['device.api']
    log:info('create ' .. api .. ' node for device: ' .. cam_data.obj_path)
    source = source or Plugin.find('standard-event-source')
    local e = source:call('create-event', 'create-' .. api .. '-device-node', cam_data.parent, nil)
...
function mutils.create_cam_nodes(self)
    ...
    for _, data in ipairs(self.cam_data) do
        ...
        if dev_ids_json:is_array() then
            ...
            data.dev_ids = {}
            for dev_id_str in dev_ids:gmatch('%S+') do --- to maintain the backward compatibility with earlier pipewire versions.
...
function mutils.register_cam_node(self, parent, id, factory, properties)
    ...
    self.cam_source = Core.timeout_add(Settings.get_int('monitor.camera-discovery-timeout'), function()
