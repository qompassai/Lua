...
SimpleEventHook({
  ...
  execute = function(event)
    ...
    local props = source:get_properties()         --- changed from local props = event:get_properties()    -- Get properties from the event *source* (WPObject) instead of the event itself,
    -- since WPEvent has no get_properties() while WPObject does.
...
function module.get_device_info(self, device)
    ...
    if not dev_info then     --- new device
        local device_name = device_properties['device.name']
        if not device_name then
            Log.warning(device, 'invalid device.name')
            return nil
        end
        Log.trace(device, string.format('create dev_info for \'%s\', object.serial = %s', device_name, device_id))
        dev_info = {
            name = device_name,
            active_profile = -1,
            route_infos = {},
